...
function Viewstudents() {
  ...
  const handleSaveChanges = async () => {
    ...
    try {
      const payload = {
        ...selectedStudent,
        phone: selectedStudent.phone ? selectedStudent.phone.toString().trim() : "",
        email: selectedStudent.email ? selectedStudent.email.toLowerCase().trim() : "",
      };
      ...
      setStudents((prev) =>
        prev.map((stu) => (stu._id === selectedStudent._id ? data : stu))
      );
      ...
      if (err.response?.status === 400) {
        ...
        toast.error("Error updating student. Please try again.", { position: "top-center" });
              ...
              <div className="d-flex flex-column flex-sm-row justify-content-between align-items-center mt-4">
                ...
                <div>
                  <Dropdown>
                    ...
                    <Dropdown.Menu>
                      {[5, 10, 20, 30, 40, 50, 60, 80, 100, 200, 300, 400, 500].map(num => (
